...
- **Enterprise-Grade Security**:
  ...
  - **Tiered secret storage** — API keys stored in GNOME Keyring (encrypted, auto-installs
    `libsecret-tools` on first use), HashiCorp Vault, or secure `chmod 600` files depending
    on environment; never written inside the git working tree
